...
For APP2 learning code, write detailed Chinese comments where they improve understanding. A useful comment explains the component's responsibility, input/output meaning, data or signal flow, ordering constraints, design reason, and the failure mode or common misconception it prevents. Do not add narration that merely repeats an obvious assignment or loop. Keep temporary test code, production BSP code, and later LVGL/DMA ownership boundaries explicit in comments.
