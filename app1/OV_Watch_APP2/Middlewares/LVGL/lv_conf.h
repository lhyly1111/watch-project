...
/*
 * APP2 的最小 LVGL 配置。
 * LVGL 8.2.0 本体来自只读 V2.4.3 参考快照；本文件位于 APP2，独立决定当前工程启用的颜色、内存和组件。
 * 第一轮只验证 LCD 显示端口和英文标签，故关闭未使用控件、主题、示例和大字体，避免它们占用 Flash/RAM 并增加排障变量。
 */
...
#define LV_MEM_CUSTOM 0
    ...
    /* LVGL 内部动态对象使用的静态内存池，单位为字节；24 KB 为最小标签页面留出空间，后续按实际对象与栈水位调整。 */
...
/* LVGL 渲染期间允许同时存在的临时内存块数量；初版页面简单，8 个可降低静态管理开销。 */
...
/* 初版只绘制纯色矩形与文字，不需要圆角、阴影、渐变和遮罩等复杂绘制。 */
